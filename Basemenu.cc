...
Basemenu::Basemenu(ToolWindow *toolwindow) {
  ...
  indicator = bbtool->getResource()->getBulletStyle();
  indicator_position = bbtool->getResource()->getBulletPosition();
  ...
  menu.width = menu.title_h = menu.item_w = menu.iframe_h =
    ...
    bbtool->getResource()->getTitleFont()->descent +
    (menu.bevel_w * 2);
  menu.label = 0;
  menu.sublevels = menu.persub = menu.minsub = 0;
  menu.item_h = bbtool->getResource()->getMenuFont()->ascent +
    bbtool->getResource()->getMenuFont()->descent + (menu.bevel_w);
  menu.height = menu.title_h + bbtool->getResource()->getBorderWidth() + 
                menu.iframe_h;
  ...
  menu.frame =
    XCreateWindow(bbtool->getXDisplay(),
                  bbtool->getCurrentScreenInfo()->getRootWindow(), 
                  menu.x, menu.y, menu.width,
		              menu.height, bbtool->getResource()->getBorderWidth(), 
  ...
  bbtool->saveMenuSearch(menu.iframe, this);
...
Basemenu::~Basemenu(void) {
  ...
  bbtool->removeMenuSearch(menu.iframe);
...
void Basemenu::update(void) {
  ...
  if (title_vis)
    menu.item_w = (menu.bevel_w * 2) +
      XTextWidth(bbtool->getResource()->getTitleFont(),
  ...
  for (; it.current(); it++) {
	...
	XTextWidth(bbtool->getResource()->getMenuFont(), *itmp->u, strlen(*itmp->u));
    else if (itmp->l)
      ii = (menu.bevel_w * 2) + (menu.item_h * 2) +
	XTextWidth(bbtool->getResource()->getMenuFont(), itmp->l, strlen(itmp->l));
    else
      ii = 0;
  ...
  menu.title_h = bbtool->getResource()->getTitleFont()->ascent +
    bbtool->getResource()->getTitleFont()->descent + (menu.bevel_w * 2);

  menu.iframe_h = (menu.item_h * menu.persub);
  menu.height = ((title_vis) ? menu.title_h + 
                  bbtool->getResource()->getBorderWidth() : 0) +
    menu.iframe_h;
  ...
  if (title_vis)
    ...
    case LeftJustify:
      {
	XDrawString(display, menu.title, bbtool->getMenuTitleGC(),

		    menu.bevel_w, bbtool->getResource()->getTitleFont()->ascent +
		    menu.bevel_w,
		    ((menu.label) ? menu.label : "Blackbox Menu"),
		    strlen(((menu.label) ? menu.label : "Blackbox Menu")));
	break;
	...
	int ins = (menu.width -
		   (XTextWidth(bbtool->getResource()->getTitleFont(),
			       ...
			       strlen(((menu.label) ? menu.label :
				       "Blackbox Menu"))))) / 2;
	XDrawString(display, menu.title, bbtool->getMenuTitleGC(), ins,
		    bbtool->getResource()->getTitleFont()->ascent + menu.bevel_w,
		    ((menu.label) ? menu.label : "Blackbox Menu"),
		    strlen(((menu.label) ? menu.label : "Blackbox Menu")));
	break;
...
void Basemenu::drawSubmenu(int index) {
  ...
  if (index >= 0 && index < menuitems->count()) {
	...
	y = (((shifted) ? menu.y_shift : menu.y) +
      ...
      if ((x + item->submenu()->getWidth()) > 
                      bbtool->getCurrentScreenInfo()->getWidth())
	x = ((shifted) ? menu.x_shift : menu.x) -
	  item->submenu()->getWidth() - bbtool->getResource()->getBorderWidth();
      if (x < 0) x = 0;

      if ((y + item->submenu()->getHeight()) > 
                      bbtool->getCurrentScreenInfo()->getHeight())
	y = bbtool->getCurrentScreenInfo()->getHeight() - 
          item->submenu()->getHeight() -
          bbtool->getResource()->getBorderWidth();
      if (y < 0) y = 0;
...
void Basemenu::drawItem(int index, Bool frame, Bool highlight, Bool clear) {
  ...
  switch(bbtool->getResource()->getMenuJustification()) {
  case LeftJustify:
  ...
  case RightJustify:
    tx = ix + menu.item_w -
      (menu.item_h + menu.bevel_w +
       XTextWidth(bbtool->getResource()->getMenuFont(),
  ...
  GC gc = (highlight || frame) ? bbtool->getMenuHiGC() :
       bbtool->getMenuFrameGC(),
     tgc = (highlight) ? bbtool->getMenuHiGC() : bbtool->getMenuFrameGC();
  ...
  if (highlight) {
    if (indicator == Round) {
      XFillArc(display, menu.iframe, bbtool->getMenuHiBGGC(), ix + 1, iy + 1,
               menu.item_h - 1, menu.item_h - 2, 90 * 64, 180 * 64);
      XFillArc(display, menu.iframe, bbtool->getMenuHiBGGC(),
               ix + menu.item_w - menu.item_h, iy + 1, menu.item_h - 1,
               menu.item_h - 2, 270 * 64, 180 * 64);
      XFillRectangle(display, menu.iframe, bbtool->getMenuHiBGGC(), ix +
                     (menu.item_h / 2), iy + 1, menu.item_w - menu.item_h,
                     menu.item_h - 1);    
    } else
      XFillRectangle(display, menu.iframe, bbtool->getMenuHiBGGC(),
                     ix + 1, iy + 1, menu.item_w - 2, menu.item_h - 2);
   
  } else if (frame) {
    if (indicator == Round)
      XFillArc(display, menu.iframe, bbtool->getMenuHiBGGC(), fx + 1, iy + 1,
  ...
  if (item->ulabel())
    XDrawString(display, menu.iframe, tgc, tx,
                iy + bbtool->getResource()->getMenuFont()->ascent + 
                (menu.bevel_w / 2) + 1,
...
void Basemenu::motionNotifyEvent(XMotionEvent *me) {
  if (me->window == menu.title && (me->state & Button1Mask)) {
	...
	if (XGrabPointer(display, menu.title, False, PointerMotionMask|
			 ...
			 None, bbtool->getMoveCursor(), CurrentTime)
	    == GrabSuccess) {
...
void Basemenu::exposeEvent(XExposeEvent *ee) {
  if (ee->window == menu.title) {
    ...
    case RightJustify:
      {
	int off = XTextWidth(bbtool->getResource()->getTitleFont(),
			     ((menu.label) ? menu.label : "Blackbox Menu"),
			     strlen(((menu.label) ? menu.label :
...
void Basemenu::enterNotifyEvent(XCrossingEvent *ce) {
  if (ce->window == menu.iframe) {
    ...
    if (menu.y + menu.height > bbtool->getCurrentScreenInfo()->getHeight()) {
      menu.y_shift = bbtool->getCurrentScreenInfo()->getHeight() - 
              menu.height - 1;
      shifted = True;
    } else if (menu.y < 0) {
      menu.y_shift = 0;
...
void Basemenu::reconfigure(void) {
  ...
  XSetWindowBorder(display, menu.frame,
		   bbtool->getResource()->getBorderColor()->getPixel());
  XSetWindowBorderWidth(display, menu.frame, 
                  bbtool->getResource()->getBorderWidth());
